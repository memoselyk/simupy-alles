...
#---- Do Clustering
data = np.array( prod_points + cons_points )

print 'Calculate distance matrix... %d ' % len(data)
d = sch.distance.pdist(data)
print 'Calculate linkage...'
L = sch.linkage(d, 
    method='complete')
    #method='average')
print 'Calculate clustering...'
idx = sch.fcluster(L, 0.2*d.max(), 'distance')
...
centroids = []
for ki in range(min(idx), max(idx)+1):
    centr,_ = kmeans( data[idx==ki], 1)
    centroids.append( centr[0] )
print 'Got %d centroids' % (len(centroids), )
...
def plot_k_mean_clustering():
    ...
    for ki in range(min(idx), max(idx)+1):
        ...
        plt.scatter( centr[0,0], centr[0,1], s=15**2, c=(0.2,0.4,0.2,0.7), marker='x', lw=2)
